...
원고 본문은 네이버 발행과 로컬 검수에만 쓴다. 발행 후 URL, 제목, 상태, 운영 판단은 `POSTING_REGISTRY.json`에 남기고 `npm run render:strategy`로 Markdown 렌더링본을 갱신한다.
...
| 블로그 통계자료 수집해 | `docs/operations/BLOG_STATS_COLLECTION_WORKFLOW.md` 기준으로 Chrome 관리자 통계창 직접 수집 |
...
사용자가 `오늘 글감 3개 뽑아줘`, `오늘 글감`, `글감 3개`처럼 짧게 말하면 아래 계약이 자동 발동한다. 사용자가 긴 체크리스트를 다시 말하게 만들지 않는다.
...
1. `오늘의 글감 포트폴리오` 표
2. `최종 글감 3개`
3. 각 글감별 근거 키워드, 고객 상황, 고객 불안/의심 문장, 기존 글 중복/회피 각도, 문장군 취급 가능 여부, 왜 오늘 봐야 하는지, 제목 후보

`오늘의 글감 포트폴리오` 표에는 아래 컬럼을 그대로 둔다.
...
- `136~138 URL대기`, 최근 발행글, 보호글과 겹치는데 중복 회피 각도를 밝히지 않으면 실패
...
- `관찰 후 작성`: 최근 발행글 또는 URL대기 글과 가까워 3일/7일 반응 확인이 먼저임

최종 글감이 `136~138 URL대기`, 최근 발행글, 보호글과 가까우면 후보 안에 `근접 경고:`를 반드시 적고, 어떤 번호/글과 어떻게 분리할지 밝힌다.
