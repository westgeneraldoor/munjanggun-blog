...
- 블로그 썸네일과 본문 사진 방향을 잡을 때는 중앙 `PHOTO_TREATMENT.md`의 `블로그 썸네일` 기준을 따른다. 제품, 전후 차이, 고객 고민 중 하나가 첫눈에 읽혀야 하며, 바깥쪽 안전영역과 개인정보 기준을 함께 확인한다.
- 썸네일/카드 이미지가 어두운 스마트스토어 썸네일 남발형, 빨간 특가 전단지형, 네이비 대기업형, AI 카드뉴스형으로 흐르면 중앙 `ANTI_PATTERNS.md` 기준으로 보정하거나 보류한다.
- 웹앱, 랜딩, 썸네일 템플릿을 직접 구현하는 작업에서는 중앙 `tokens/brand.css` 또는 `tokens/brand.tokens.json`을 우선 사용한다. 블로그 원고 작성만 할 때는 토큰을 억지로 적용하지 않는다.
...
12. ★★★ docs/operations/APPSHEET_FIELD_STORY_WORKFLOW.md 기준으로 현장 조건과 AppSheet 후매칭 카드를 설계한다.
    ...
    - 썸네일/본문 사진은 중앙 `PHOTO_TREATMENT.md`의 개인정보, 수직선, 밝기, 블로그 썸네일 안전영역 기준을 따른다.
    - 사진/썸네일 톤은 중앙 `ANTI_PATTERNS.md`로 검수한다. 어두운 판매형 썸네일, AI 카드뉴스형, 과한 특가 전단지형은 사용하지 않는다.
