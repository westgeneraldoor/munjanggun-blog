...
- 원고 작성 완료와 URL 등록도 다른 상태다. 검수 완료본을 직원에게 전달해 등록부에 남긴 순간부터 해당 글은 `작성완료·URL등록대기`이며, URL이 없어도 중복/카니발 방지 대상이다.
...
Codex가 원고를 완료하면, 직원의 네이버 포스팅과 URL 수신을 기다리는 동안에도 먼저 `POSTING_REGISTRY.json`에 파일·제목·키워드·소재 요약과 `작성완료·URL등록대기`를 남긴다. 이 상태는 URL만 없는 콘텐츠 완료본이며 새 글감으로 다시 쓰지 않는다.

직원이 네이버에 포스팅하고 사용자가 URL을 주면:

1. `docs/strategy/POSTING_REGISTRY.json`에 URL 등록 후 렌더링본 갱신
2. 최근 작성본 상태를 `발행완료·URL등록완료`로 변경
3. 다음 데일리 통계 자료에 발행 링크 포함
4. URL 등록 후 같은 글은 `POST_ALREADY_PUBLISHED`로 재발행 차단

`POST_ALREADY_PUBLISHED`는 같은 파일의 재발행을 막는 발행 게이트다. 다른 신규 글감과의 소재 중복은 그보다 앞서, `작성완료·URL등록대기`를 포함한 등록부 전체로 막는다.
