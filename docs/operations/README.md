...
npm run ops:daily          # daily report/topic scorecard/키워드 데이터 점검
npm run ops:weekly         # ranking/top10/tracking history 주간 보조 점검
...
- 리포트: `outputs/reports/`
- 신규 글감 scorecard: `outputs/reports/topic_candidates/`
...
1. 새 글 작성 전 `docs/OPERATING_INDEX.md`에서 작업 유형별 필수 문서를 먼저 확인합니다.
2. 신규 글감은 `docs/operations/TOPIC_SELECTION_SCORECARD.md` 기준으로 네이버 광고 API 시장 수요와 블로그 실제 반응을 함께 본 뒤 문장군 필터를 통과한 주제만 후보로 올립니다. 후보 판단은 `outputs/reports/topic_candidates/YYYY-MM-DD_topic_scorecard.md` 형식으로 남깁니다.
3. 신규 글 작성인지 리라이팅인지 애매하면 `docs/operations/CONTENT_WORKFLOW_PLAYBOOK.md`의 보호 글/리라이팅 큐를 먼저 확인합니다.
