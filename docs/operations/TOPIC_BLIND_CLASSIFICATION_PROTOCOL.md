...
생성기는 날짜뿐 아니라 `TOP20 진입`, `보호 자산`, `순위 이탈 보강`, 큐 ID, 작성 템플릿처럼 결과·운영 방식을 암시하는 문장을 제거한다. 등록부의 실제 글번호가 `번` 또는 내부링크 번호열 문맥에 등장하면 `[다른 글]`로 바꾼다. 정제 뒤 이런 단서가 남으면 출력 파일을 쓰지 않고 실패해야 한다.
