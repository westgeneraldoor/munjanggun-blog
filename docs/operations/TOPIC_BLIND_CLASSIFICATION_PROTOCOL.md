...
`has_summary`는 `topic_summary` 또는 `target_keywords` 중 하나라도 있으면 `true`, 둘 다 비어 제목만 있으면 `false`다. 분류 라벨이 아니며, `false`인 항목은 제공 정보가 적으므로 필요한 경우 `confidence: low`를 사용한다. `[날짜 마스킹]`은 입력에서 제거된 날짜의 대체 표기이므로 소재 특징으로 해석하지 않는다.
